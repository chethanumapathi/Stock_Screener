...
def _get_quarterly_profit_history(symbol):
...
def is_profit_doubled_as_of(quarters_list, as_of_date_str):
    """
...
def backtest(df):
    ...
    if df is None or len(df) < 30:
        return {'long_entry': pd.Series(False, index=df.index if df is not None else []), 'tp_pct': 1.0, 'sl_pct': 0.15, 'trades': []}
...
def run_test(segment_name):
    print(f"\n==========================================")
    print(f"Running Backtest on {segment_name.upper()}...")
    print(f"==========================================")
    t0 = time.time()
    req_body = json.dumps({
        'code': strategy_code,
    ...
    print(f"Results for {segment_name.upper()} ({time.time()-t0:.2f}s):")
    print(f"  Status: {res.get('status')}")
    print(f"  Total Trades: {res.get('total_trades')}")
    print(f"  Net P&L: Rs {res.get('net_pnl')}")
    print(f"  Win Rate: {res.get('win_rate_pct')}%")
    print(f"  Profit Factor: {res.get('profit_factor')}")
    print(f"  Duration: {res.get('duration_seconds')}s")

    trades = res.get('trades', [])
    closed = [t for t in trades if t.get('exit_reason') != 'End of Data']
    running = [t for t in trades if t.get('exit_reason') == 'End of Data']
    print(f"  Closed Trades: {len(closed)} | Running Trades: {len(running)}")
    if closed:
        print("\nTop 5 Closed Trades by P&L:")
        sorted_closed = sorted(closed, key=lambda x: float(x.get('pnl_pct') or 0), reverse=True)
        for t in sorted_closed[:5]:
            print(f"    {t.get('symbol')} | {t.get('entry_date')} @ {t.get('entry_price')} -> {t.get('exit_date')} @ {t.get('exit_price')} | PnL: +{t.get('pnl_pct')}% | {t.get('exit_reason')}")
